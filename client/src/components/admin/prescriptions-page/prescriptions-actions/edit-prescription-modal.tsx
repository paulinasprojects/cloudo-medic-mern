...
import EditPresctiptionForm from "./edit-prescription-form"
...
export default function EditPrescriptionModal({ isOpen, onClose, data }: Props) {
    ...
    <div onClick={handleClickOutside} className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-black rounded-sm border border-slate-700 w-full max-w-xl">
        <div className="flex items-center justify-between px-4 py-5">
          <h2 className="text-lg font-bold text-gray-500">Edit Prescription</h2>
        ...
        <EditPresctiptionForm onSuccess={handleClose} data={data}/>
      </div>
    </div>
  )
}
