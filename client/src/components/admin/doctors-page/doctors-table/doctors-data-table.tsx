...
import { DropdownMenu, DropdownMenuContent, DropdownMenuCheckboxItem, DropdownMenuTrigger } from "@/components/common/dropdown-menu";
  ...
  return (
    <div>
      <div className="flex items-center py-4">
        <input
          placeholder="Find by education"
          value={(table.getColumn("education")?.getFilterValue() as string) ?? ""}
          onChange={(event) => table.getColumn("education")?.setFilterValue(event.target.value)}
                    ...
                    <TableHead key={header.id} style={{ width: header.getSize() }} className="relative" >
                      ...
                      {header.column.getCanResize() && (
                        <div
                          ...
                          className={cn(
                            "absolute right-0 top-0 h-full flex items-center cursor-col-resize select-none touch-none px-0.5",
              ...
              table.getRowModel().rows.map((row) => (
                <TableRow key={row.id} data-state={row.getIsSelected() && "selected"}>
                  {row.getVisibleCells().map((cell) => (
                    <TableCell key={cell.id} className="whitespace-break-spaces" style={{ width: cell.column.getSize() }}>
